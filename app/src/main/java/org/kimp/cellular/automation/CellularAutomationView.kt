...
class CellularAutomationView : View {
    ...
    private fun applyAttributes(@NonNull context: Context, attrs: AttributeSet) {
        ...
        backgroundColor = Color.valueOf(
            a.getColor(R.styleable.CellularAutomationView_backgroundColor, backgroundColor.toArgb())
        )
        cellColor = Color.valueOf(
            a.getColor(R.styleable.CellularAutomationView_cellColor, cellColor.toArgb())
        )
    ...
    private fun checkCell(cell: Pair<Int, Int>): Int {
        ...
        return if (neighbourCounter == 3 && !currentCells.contains(cell)) lifeCell
        else if (neighbourCounter in 2..3 && currentCells.contains(cell)) lifeCell
        else emptyCell
    }

    override fun onTouchEvent(event: MotionEvent?): Boolean {
        if (event?.action == MotionEvent.ACTION_DOWN) {
            var newPoint: Pair<Int, Int> = Pair(
                event.x.toInt() / cellSize,
                event.y.toInt() / cellSize
            )
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (cell in currentCells) {
            var currentRect: Rect = Rect(
                ...
                (cell.first + 1) * cellSize,
                (cell.second + 1) * cellSize
            )
